...
class QueueService:
    ...
    async def get_task(self, task_id: str) -> Optional[QueueTask]:
        ...
        try:
            ...
            if row:
                task_data = json.loads(row[1])  # task_data 列
    ...
    async def _persist_task(self, task: QueueTask):
        ...
        try:
            ...
            # 檢查任務是否已存在
            result = await cursor.execute(
                "SELECT COUNT(*) FROM queue_tasks WHERE task_id = ?",
                (task.task_id,)
            )
            ...
            if exists:
                await cursor.execute(
                    """UPDATE queue_tasks 
                       SET task_data = ?, updated_at = CURRENT_TIMESTAMP 
                       WHERE task_id = ?""",
                    (task_data, task.task_id)
                )
            else:
                await cursor.execute(
                    """INSERT INTO queue_tasks (task_id, task_data, created_at, updated_at)
        ...
        try:
            ...
            result = await cursor.execute(
                """SELECT task_data FROM queue_tasks 
                   WHERE JSON_EXTRACT(task_data, '$.status') IN ('pending', 'processing')
                   ORDER BY JSON_EXTRACT(task_data, '$.priority'), 
                           JSON_EXTRACT(task_data, '$.created_at')"""
            )
            ...
            logger.info(f"從資料庫恢復任務: {restored_count} 個")
        ...
        try:
            ...
            result = await cursor.execute(
                """DELETE FROM queue_tasks 
                   WHERE JSON_EXTRACT(task_data, '$.completed_at') < ? 
                   AND JSON_EXTRACT(task_data, '$.status') IN ('completed', 'failed', 'cancelled')""",
