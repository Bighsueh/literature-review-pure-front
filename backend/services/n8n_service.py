...
class N8NService:
    ...
    async def extract_keywords(
        ...
        try:
            ...
            # 1. 格式: [{"output": {"keywords": [...]}}]
            if (
                isinstance(result, list)
                and len(result) > 0
                and isinstance(result[0], dict)
                and 'output' in result[0]
                and isinstance(result[0]['output'], dict)
                and 'keywords' in result[0]['output']
            ):
            ...
            if keywords:
                ...
                logger.warning(f"無法解析關鍵詞提取回應格式: {result}")
                return {"error": "回應格式異常", "raw_response": result}
        ...
        try:
            ...
            if all(field in actual_result for field in expected_fields):
                ...
                if 'response' in actual_result:
                    # 如果至少有 response 欄位，建構標準格式
                    standardized_result = {
                        'response': actual_result.get('response', ''),
                        'references': actual_result.get('references', []),
                        'source_summary': actual_result.get('source_summary', {
                            'total_papers': 0,
                            'papers_used': []
                        })
                    }
                    logger.info(f"統一內容分析完成（標準化格式），回應長度: {len(standardized_result['response'])}")
                    return standardized_result
                else:
                    logger.warning(f"統一內容分析回應格式異常，實際欄位: {list(actual_result.keys())}")
                    return {"error": "回應格式異常", "raw_response": result}
