...
CREATE TABLE IF NOT EXISTS paper_sentences (
    ...
    detection_status TEXT DEFAULT 'unknown', -- success, error, unknown
    error_message TEXT, -- 檢測失敗時的錯誤信息
...
CREATE INDEX IF NOT EXISTS idx_sentences_detection_status ON paper_sentences(detection_status);
CREATE INDEX IF NOT EXISTS idx_sentences_retry_count ON paper_sentences(retry_count);
